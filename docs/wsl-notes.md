...
```
cd ~
echo 'sudo apt update && sudo apt upgrade -y && sudo apt autoremove -y' > bin/run-updates.bash && chmod +x bin/run-updates.bash
...
```
udo apt-get install apt-transport-https ca-certificates gnupg
...
- See: https://github.com/nvm-sh/nvm#git-install for latest version to install.
```
cd ~
